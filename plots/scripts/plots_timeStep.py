    ...
    for m in range(len(timeStepTypes)):
        ...
        for n in range(len(stiffnessTypes)):
            ...
            for integrator in integratorTypes:
                filePath = filePathPrefix + timeStepTypes[m] + '_' + stiffnessTypes[n] + '_numSeg_10' + '/RandomYoungs/' + materialTypes[mt] + '/' + integrator + fileName
                print(filePath)
        ...
        for j in range(len(plotsType)):
            ...
            for m in range(len(timeStepTypes)):
                for n in range(len(stiffnessTypes)):
                    ...
                    plt.plot(integratorDataset[mt][m][n][i][0:plotSeconds, 0], integratorDataset[mt][m][n][i][0:plotSeconds, 1 + j], linewidth=0.5, label = 'randomYoungs' + '_' + materialTypes[mt] + '_' + integratorTypes[i] + '-h=' + timeStepLabels[m] + '-IPC_K=' + stiffnessLabels[n])
                    plt.legend()
                    imgPath = filePathPrefix + timeStepTypes[m] + '_' + stiffnessTypes[n] + '_numSeg_10' + '/bottomDis/' + 'randomYoungs_' + materialTypes[mt] + '_' + integratorTypes[i] + '_' + plotsType[j] + '.png'
                    plt.savefig(imgPath)